...
internal static class RowMappers
{
    ...
    public static AuditEvent ToDomain(AuditEventRow row) =>
        new(
            ...
            row.Channel is { } channel ? (AuditChannel)channel : null,
    ...
    public static AuditEventRow ToRow(AuditEvent auditEvent) => new()
    {
        ...
        Channel = (int?)auditEvent.Channel,
    ...
    private static Provenance ToDomain(ProvenanceJson json) =>
        new(
            ...
            json.IsAiGenerated,
            json is { AiMarkedBy: { } markedBy, AiMarkedAt: { } markedAt, AiMarkedReason: { } reason }
                ? new AiMarking(new UserId(markedBy), markedAt, reason)
                : null);
    ...
    private static ProvenanceJson ToRow(Provenance provenance) => new()
    {
        ...
        AiMarkedBy = provenance.AiMarking?.MarkedBy.Value,
        AiMarkedAt = provenance.AiMarking?.MarkedAt,
        AiMarkedReason = provenance.AiMarking?.Reason,
        Evidence =
        [
