...
**Status: IN PROGRESS.** The owner chose option 3 on 2026-09-26 (`info.md`): no domain, an
internal CA, and Caddy in LXC 100. The gateway runs and the LAN's plain HTTP port is closed. What
is left is each device trusting the root, which is the owner's to do, and then HSTS.
