...
| `win-arm64` | yes | **client only, every release** | **Client-verified since 2026-09-24** (`info.md`), first run against `v1.6.0`: the console and `DevBuddy.McpServer --stdio`, from the published archive, in the Windows on ARM VMware guest on Apple silicon. The API in the same archive is **not supported** on this RID. Every earlier release was backfilled on 2026-09-22 (console only). |
...
| `win-arm64` client smoke test | **Required since 2026-09-24**, run after publication | **Yes, in a VM**, on 2026-09-24: Windows on ARM 10.0.26200.9457, `ARM64`, in the VMware guest on Apple silicon. The archive was downloaded; its SHA-256 matched `SHA256SUMS`, and its attestation verified against `refs/tags/v1.6.0` and `c9a0d4d…`, with a wrong-owner control refused. Both apphosts are PE `0xAA64`. **Console:** twenty operations, identical to the console image; `--every 24` exited 2; `migrate` with no connection string exited 2. **MCP server, `--stdio`**, pointed at no database: answered `initialize` and `tools/list` with the same twenty names, wrote only JSON-RPC to standard output, and exited 0 when its input closed. The API was not run: it is not supported on this RID. **The RID is client-verified from this run on.** |
...
Tag `v1.5.0` pushed from `cb818a3`, run 35685837379, 2026-09-22. Every job passed on the first
attempt. Between `50ccaaa` and `cb818a3` only this file and `docs/plan-phase-13.md` changed. The
owner approved downloading the archives for this checklist, `win-arm64` excepted, and publishing
once it passed (`info.md`, 2026-09-22). **Published on 2026-09-22, 04:22 UTC**, as Latest.
...
| Images **started** on `linux/arm64` | Re-run, against the published images | **Yes, on arm64:** the Ubuntu 26.04.1 VMware guest on the Apple M4 (`aarch64`). The same script, checks and results, with `arm64` and the same digests. |
| `win-x64` smoke test | Re-run | **Yes, natively**, on the development machine: Windows 11 Pro 10.0.26200 on AMD64. Twenty operations, identical to the console image; `--every 24` refused with exit 2. Smart App Control read Off. |
| `linux-x64` smoke test | Re-run | **Yes.** `ubuntu:24.04` (24.04.4) with `libicu74`, `x86_64`, on jmhp. Twenty operations, identical; `--every 24` refused. |
| `linux-musl-x64` smoke test | Re-run | **Yes.** `alpine:3` (3.24.1) with `libstdc++`, `libgcc` and `icu-libs`, `x86_64`, on jmhp. Twenty operations, identical; `--every 24` refused. |
| `osx-arm64` smoke test | Re-run | **Yes, on hardware.** Natively on the Apple M4 Mac mini, macOS 26.6.2. Twenty operations, identical; `--every 24` refused; the apphost is an ad-hoc signed arm64 Mach-O. |
...
| `linux-musl-x64` smoke test | Re-run | **Yes.** `alpine:3` (3.24.1) with `libstdc++`, `libgcc` and `icu-libs`, `x86_64`, on jmhp. Twenty operations, identical; `--every 24` refused. |
| `osx-arm64` smoke test | Re-run | **Yes, on hardware.** Natively on the Apple M4 Mac mini, macOS 26.6.2. Twenty operations, identical; `--every 24` refused; the apphost is an ad-hoc signed arm64 Mach-O. |
| `linux-arm64` smoke test | Re-run | **Yes, in Docker Desktop's Linux VM on the Apple M4:** `ubuntu:24.04` (24.04.5), `aarch64`. Twenty operations, identical; `--every 24` refused. **Then natively, after publication (2026-09-17):** the published archive, checksum matched, on Ubuntu 26.04.1 in the VMware guest on the Apple M4, `aarch64`, 2 CPUs, `libicu78`. Twenty operations, identical; `--every 24` refused with exit 2. The guest had been switched off when the rest of the checklist ran. |
