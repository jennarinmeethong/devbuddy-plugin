...
| 2026-09-24 | v1.6.0 published | Tag `v1.6.0` was pushed from `c9a0d4d`. Release run 35957756037 passed every job on the first attempt. **The post-tag checklist passed in full:**<ul><li>checksums, and attestations checked from outside the workflow for six archives and three images, with wrong-owner controls refused;</li><li>SBOMs of 36, 38 and 56 components;</li><li>both architectures under one digest per tag.</li></ul>**Published images started** on amd64 on jmhp and on arm64 in the Ubuntu guest, each with nine migrations. **Smoke tests passed** for `win-x64` natively, `linux-x64`, `linux-musl-x64`, `osx-arm64` natively on the M4, `linux-arm64` natively in the guest, and `linux-musl-arm64` in Alpine there. It was **published** at 05:11 UTC as Latest, at the owner's instruction.<br>**The devbox:** backup `backup-20260924-051207-32e2cde7279149a4a` and `.env` were copied to `/data/devbuddy-cache/backups/before-v160-20260924/`. It was checked out at `v1.6.0`, rebuilt and recreated with the workers profile. No migration ran, and nine are in the history table. `/health` answered 200 and an MCP `POST` 401. `scope-report` was clean. `embedding-check` was all ok, with `SelfHosted` on `http://ollama:11434/v1`, so the new private-address rule passed the installation's own model server. Both workers completed a pass; the embedding sweep skipped three drafts, and the index is empty because its one published record was archived. Every service runs as its non-root user on the new image. **Plugin 1.6.0** was installed, and `list_projects` answered through a fresh MCP stdio on the new image. |
| 2026-09-24 | `win-arm64` as a client | The owner decided that `win-arm64` is a client platform (`info.md`). Every release now owes a smoke test of the console and of `DevBuddy.McpServer --stdio`, and the API in the archive is not supported there. **The test ran against the published `v1.6.0` archive**, in the Windows on ARM guest. The checksum and attestation were checked, with a wrong-owner control refused. Both apphosts are ARM64. The console listed twenty operations identical to the console image and refused `--every 24` and a missing connection string with exit 2. The MCP server over stdio, with no database, answered `initialize` and `tools/list` with the same twenty names, wrote only JSON-RPC to stdout, and exited 0. The RID is client-verified, and no RID is left unverified. |
